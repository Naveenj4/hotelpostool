...
import { Download, Loader2, Calendar, Search, Activity, PieChart } from 'lucide-react';
...
const DayWiseSales = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const url = `${import.meta.env.VITE_API_URL}/reports/weekly?startDate=${dateRange.start}&endDate=${dateRange.end}`;
    ...
    const exportToCSV = () => {
        ...
        const headers = "Date,Total Bills,Total Sales\n";
        const rows = data.map(row => `${row.date},${row.billCount},${row.totalSales}`).join('\n');
        const csvContent = "data:text/csv;charset=utf-8," + headers + rows;
        const encodedUri = encodeURI(csvContent);
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", `Day_Wise_Sales_${dateRange.start}_to_${dateRange.end}.csv`);
    ...
    const chartOptions = {
        ...
        onClick: (event, elements) => {
            if (elements.length > 0) {
                ...
                navigate('/dashboard/self-service/bills-sales', { state: { date: label } });
            }
        }
    };
                ...
                <div className="master-content-layout fade-in">
                    <div className="master-header-premium">
                        <div className="master-title-premium">
                            ...
                            <h2>Day-wise Sales Archive</h2>
                            <p>Daily breakdown of revenue and volume across operations.</p>
                        </div>
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center p-20">
                            ...
                            <p className="text-slate-400 font-bold tracking-widest uppercase text-xs">Assembling Daybook Insights...</p>
                        </div>
                    ) : (
                                    ...
                                    <table className="table-premium">
                                        ...
                                        <tbody>
                                            {data.length === 0 ? (
                                                ...
                                                data.map((day, ix) => (
                                                    <tr key={ix} 
                                                        onClick={() => navigate('/dashboard/self-service/bills-sales', { state: { date: day.date } })}
                                                    ...
                                                    >
                                                        ...
                                                        <td className="text-right font-black text-indigo-600">
                                                            ₹{day.totalSales.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
